...
package io.github.vino42.base.xss;
...
public class XssUtils {
    ...
    private Policy createPolicy() {
        try {
            ...
            log.warn("[YiShu] |- Antisamy create policy error! {}", e.getMessage());
            return null;
        }
    }
    ...
    private String cleanHtml(String taintedHtml) {
        try {
            log.trace("[YiShu] |- Before Antisamy Scan, value is: [{}]", taintedHtml);

            // 使用AntiSamy清洗数据
            final CleanResults cleanResults = scan(taintedHtml);
            String result = cleanResults.getCleanHTML();
            log.trace("[YiShu] |- After  Antisamy Scan, value is: [{}]", result);
            return result;
        } catch (ScanException | PolicyException e) {
            log.error("[YiShu] |- Antisamy scan catch error! {}", e.getMessage());
            return taintedHtml;
        }
    }
    ...
    public static String cleaning(String taintedHTML) {
        ...
        log.trace("[YiShu] |- After  Antisamy Well Formed, value is: [{}]", result);
        return result;
    }
}
