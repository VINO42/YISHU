...
public class MinioAsyncClientObjectPool {
    ...
    public MinioAsyncClient getMinioClient() throws OssErrorException {
        try {
            MinioAsyncClient minioAsyncClient = minioAsyncClientPool.borrowObject();
            log.debug("[YISHU] |- Fetch minio async client from object pool.");
            return minioAsyncClient;
        } catch (Exception e) {
            log.error("[YISHU] |- Can not fetch minio client from pool.");
            throw new OssErrorException();
        }
    }
